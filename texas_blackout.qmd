---
title: "EDS-223: 2021 Texas Blackout and Income Disparity"
...
# Create a bounding box for Houston to apply to lights images
...
# Set CRS to be equal between road data and blackout mask object
roads <- st_transform(roads, crs = st_crs(blackout_projected))

# Aggregate roads data to make processing faster
roads_union <- st_union(roads)

# Create buffer object of 200 m around roads
roads_buffer <- st_buffer(roads_union, 200)

# Join road data with blackout mask by areas that are not within buffer
blackout_roads <- st_difference(blackout_projected, roads_buffer)

# Set CRS of residential data to be the same as the buffered blackout data
houses <- st_transform(houses, crs = st_crs(blackout_roads))

# Clip to shared residential data and blackout mask with road buffer
...
# Aggregate residential blackout data for faster processing
blackout_res_union <- st_union(blackout_res)

# Join data between census and blackout residential data
socio_spatial <- st_filter(socio, blackout_res_union)

# Subset joined data to include only median income and spatial grouping 
socio_subset <- socio_spatial %>% 
  select(B19013e1, COUNTYFP, TRACTCE)

# Map census tracts that were impacted by blackout
